<script type="text/javascript">
  $(document).ready(function(){
    $('#series_ott_platforms').DataTable({
      'paging'      : true,
      'lengthChange': true,
      'searching'   : true,
      'ordering'    : true,
  ...
  function updateItem(id){
    ...
    if(name=="" && name!=undefined){
        alert("Please enter the Series OTT Platform Name");
    ...
    $.ajax({
        type: 'POST',
        url: '<?= BASE_URL ?>series_ott_platforms/updateItem',
  ...
  function addItem(){
    var table = document.getElementById("series_ott_platforms");
    
    var latest_row_id = $('#series_ott_platforms tr:last').attr('id');
  ...
  function add(id){
    ...
    $.ajax({
        type: 'POST',
        url: '<?= BASE_URL ?>series_ott_platforms/addItem',
        data: { id: id, name: name, order_no: order },
        beforeSend: function() {
            $(this).html('Adding...');
        },
  ...
  function cancelUpdate(id){
    $.ajax({
      type: 'POST',
      url: '<?= BASE_URL ?>series_ott_platforms/getItem',
      data: { id: id },
      success: function(response) {
  ...
  function deleteItem(id){
    var result = confirm("Do you really want to delete this Series OTT Platform?");
    if (result) {
      $.ajax({
        type: 'POST',
        url: '<?= BASE_URL ?>series_ott_platforms/deleteItem',
        data: { id: id },
        beforeSend: function() {
          $(this).html('Deleting...');
        },
