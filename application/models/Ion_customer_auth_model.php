 ...
 */
class Ion_customer_auth_model extends CI_Model
{
	/**
	...
	/**
	 * caching of customers and their groups
	 *
	 * @var array
	 */
	 ...
	 * against an entry in the customers table.
	 *
	 * @param string	$password
	...
	public function check_password($password, $hash_password_db, $identity = NULL)
	{
		// Check for empty id or password, or password containing null char, or password above limit
		// Null char may pose issue: http://php.net/manual/en/function.password-hash.php#118603
		// Long password may pose DOS issue (note: strlen gives size in bytes and not in multibyte symbol)
			
		/*if (empty($password) || empty($hash_password_db) || strlen($password) > self::MAX_PASSWORD_SIZE_BYTES)
		{
			return FALSE;
		}
*/
		if(base64_encode($password)==$hash_password_db){
		 	return TRUE;
		}else{
			return FALSE;
		}
	...
	public function get_user_by_activation_code($user_code)
	{
		...
		if ($token) 
		{
			...
			$user = $this->where('activation_selector', $token->selector)->customers()->row();
	...
	public function activate($id, $code = FALSE)
	{
		...
		if ($code === FALSE || ($user && $user->id === $id))
		{
			...
			$this->db->update($this->tables['customers'], $data, ['id' => $id]);
	...
	/**
	 * Updates a customers row with an activation code.
	 *
	...
	public function clear_forgotten_password_code($identity) {
		...
		$this->db->update($this->tables['customers'], $data, [$this->identity_column => $identity]);
	...
	public function forgotten_password($identity)
	{
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['customers'], $update, [$this->identity_column => $identity]);
	...
	public function get_user_by_forgotten_password_code($user_code)
	{
		...
		if($token) {
			// Retrieve the user according to this selector
			$user = $this->where('forgotten_password_selector', $token->selector)->customers()->row();
	...
	public function register($identity, $password, $email, $additional_data = [], $groups = [])
	{
		...
		// filter out any data passed that doesnt have a matching column in the customers table
		// and merge the set user data and the additional data
		$user_data = array_merge($this->_filter_data($this->tables['customers'], $additional_data), $data);

		$this->trigger_events('extra_set');

		$this->db->insert($this->tables['customers'], $user_data);

		$id = $this->db->insert_id($this->tables['customers'] . '_id_seq');
	...
	public function login($identity, $password, $remember=FALSE){
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($this->check_password($password, $user->password, $identity))
			{
	 ...
	 * @author Ben Edmunds
	 */
	public function customers($groups = NULL)
	{
		$this->trigger_events('customers');
		...
		else
		{
			...
			$this->db->select([
			    ...
			    $this->tables['customers'].'.id as id',
			    $this->tables['customers'].'.id as user_id'
			]);
		}
		...
		if (isset($groups))
		{
			...
			if (isset($groups) && !empty($groups))
			{
				...
				$this->db->join(
				    ...
				    $this->tables['users_groups'].'.'.$this->join['customers'].'='.$this->tables['customers'].'.id',
				    'inner'
				);
			}
		...
		$this->response = $this->db->get($this->tables['customers']);
	...
	public function user($id = NULL)
	{
		...
		// if no id was passed use the current customers id
		$id = isset($id) ? $id : $this->session->userdata('user_id');

		$this->limit(1);
		$this->order_by($this->tables['customers'].'.id', 'desc');
	...
	public function get_users_groups($id = FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description')
		                ->where($this->tables['users_groups'].'.'.$this->join['customers'], $id)
		...
		foreach ($group_ids as $group_id)
		{
			...
			if ($this->db->insert($this->tables['users_groups'],
								  [ $this->join['groups'] => (float)$group_id,
									$this->join['customers']  => (float)$user_id  ]))
			{
	...
	public function remove_from_group($group_ids = FALSE, $user_id = FALSE)
	{
		...
		if (!empty($group_ids))
		{
			...
			foreach ($group_ids as $group_id)
			{
				...
				$this->db->delete(
					$this->tables['users_groups'],
					[$this->join['groups'] => (float)$group_id, $this->join['customers'] => (float)$user_id]
				);
		...
		{
			// Cast to float to support bigint data type
			if ($return = $this->db->delete($this->tables['users_groups'], [$this->join['customers'] => (float)$user_id]))
			{
	...
	public function update($id, array $data)
	{
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['customers'], $data, ['id' => $user->id]);
	...
	public function delete_user($id)
	{
		...
		// delete user from customers table should be placed after remove from group
		$this->db->delete($this->tables['customers'], ['id' => $id]);
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['customers'], ['last_login' => time()], ['id' => $id]);
	...
	public function remember_user($identity)
	{
		...
		{
			$this->db->update($this->tables['customers'],
	...
	public function login_remembered_user()
	{
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($this->verify_password($token->validator, $user->remember_code, $identity))
			{
				...
				// extend the customers cookies if the option is enabled
