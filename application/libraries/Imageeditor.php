...
class Imageeditor {
  ...
  function Initialize($filename, $path, $col=NULL) 
  {
    $this->font = false;
    $this->error = false;
    $this->size = 25;
    ...
    ## IF NO IMAGE SPECIFIED CREATE BLANK IMAGE
    {
     ...
     /* if(is_array($col)) 
      ...
      {*/
		...
		$colour = imagecolorallocate($this->img, 255,255,240);
    ...
    ## IMAGE SPECIFIED SO LOAD THIS IMAGE
    {
      ...
      if(!($this->error)) 
      {
        ...
        $extension=strtolower($file_split[1]);
  ...
  ########################################################
  function resize($width, $height, $refHeightWeight = NULL) 
  {
    if(!$this->error) 
    {
		/****************** Added by Shreebatsa************************/
		...
		}else if($refHeightWeight = "w")
		{
			$scale = $size[0]/$width;
		}else if($refHeightWeight = "h")
		{
			$scale = $size[0]/$height;
		}
		if ($scale == 0)
		...
		$width = ceil($size[0]/$scale) ;
		$height = ceil($size[1]/$scale) ;
		/*************************************************************/
  ...
  function errorImage($str) 
  {
    ...
    $this->type = "jpg";
    $this->img = @imagecreatetruecolor($this->x, $this->y);
    $this->addText("No Image", 20, 45, array(255,255,255));
